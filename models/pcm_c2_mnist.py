...
class PCM_c2(nn.Module):
    def __init__(self, x_dim, h_dim, z_dim, px_dim, pz_dim, qz_dim, n_layers, bias=False):
        ...
        self.dec = nn.Sequential( nn.Linear(self.x_dim + 2*self.z_dim + 2*self.h_dim , self.px_dim ),
                                nn.ReLU())
    ...
    def decode (self,x, phi_z_t , h,  h_1, z_t_1):
        phi_z_t = torch.cat( [x, phi_z_t , h, h_1, z_t_1 ] , 1)
    ...
    def forward(self, x):
        ...
        z_t = torch.zeros(x.size(1), self.z_dim)
        z_t_sampled = []
        ...
        for t in range(x.size(0)):
            ...
            #decoder
            if t==0:
                dec_mean_t , dec_std_t = self.decode(0*x[t-1], phi_z_t , h,  0*h_1, 0*z_1)
            else:
                dec_mean_t , dec_std_t = self.decode(x[t-1], phi_z_t , h,  h_1, z_1)
    ...
    def log_likelihood_2(self, x, n_samples):
        ...
        h_1_ = torch.zeros(n_samples,x.size(1), self.h_dim)
        
        h_bis = torch.zeros(n_samples,x.size(1), self.h_dim)
        h_1_bis = torch.zeros(n_samples,x.size(1), self.h_dim)
    ...
    def log_likelihood_(self, x_, n_samples):
        ...
        z_t = torch.zeros(n_samples, self.z_dim)
...
#         print('h_bis.shape',h_bis.shape)
#         print('x.shape',x.shape)
        
        for t in range(x.size(0)):
            phi_x_t = self.phi_x(x[t])
#             print('phi_x_t shape',phi_x_t.shape)
            #----------------------------------------------
            #prior
...
#             print('h shape',h.shape)
            ...
            prior_t = self.prior(h)
#             print('prior shape', prior_t.shape)
            prior_mean_t = self.prior_mean(prior_t)
            prior_std_t = self.prior_std(prior_t)
            p_prior = Norm.Normal(prior_mean_t, prior_std_t)
...
#             print('logp_decoder',logp_decoder.shape)
            logp_encoder = p_encoder.log_prob(z_t).sum(dim=1)
#             print('logp_encoder', logp_encoder.shape)
            logp_prior = p_prior.log_prob(z_t).sum(dim=1)
#             print('logp_prior', logp_prior.shape)
            h_bis = h
            #-------------------------------------------------------------------------    
            h_ = h_bis.detach().clone()
            h_1_ = h_1_bis.detach().clone()
...
#             print('log_w',log_w.shape )
    ...
    def sample(self, seq_len, device):
        ...
        z_t = torch.zeros(1, self.z_dim)
        
        for t in range(seq_len):
            z_1 = z_t.detach().clone()
            prior_t = self.prior(h)
            prior_mean_t = self.prior_mean(prior_t)
            prior_std_t = self.prior_std(prior_t)
            ...
            #print('t',t)
            #print('x_t',x_t)
            dec_mean_t , dec_std_t = self.decode(x_t, phi_z_t , h, h_1, z_1)
